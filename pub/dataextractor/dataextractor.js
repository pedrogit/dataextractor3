...
// 1) test empty
var vals3 = generateCSV();
if (vals3 != '') {
  throw new Error("Test 1 failed!");
}

// 2) test with source only
vals3 = generateCSV('aa');
if (vals3 != '') {
...
vals3 = generateCSV(source, newf1);
if (vals3 != 'f1;\n') {
...
vals3 = generateCSV(source, newf1); // 'f1;f2;f3;\na;b;c;\ne;;;\n'
if (vals3 != 'f1;f2;f3;\na;;c;\ne;;;\n') {
...
vals3 = generateCSV(source, fieldSet); // 'f1;f2;\na;b;\n'
if (vals3 != 'f1;f2;\nAA;BB;\n') {
...
vals3 = generateCSV(source, fieldSet);
if (vals3 != 'f1;f2;\ny;;\n') {
...
var formatNewRow = (addInputEvent = true) => {
  ...
  // add the delete event listener to all buttons
  /*
  var deleteButtons = newRow.getElementsByClassName('deleteRowButton');
  Array.from(deleteButtons).forEach(function (element) {
    element.addEventListener('click', alert('deleteRow'));
  });
  
  var deleteButtons = parent.getElementsByClassName('deletesectionbutton');
  Array.from(deleteButtons).forEach(function (element) {
    element.removeEventListener('click', handleFieldRemoved);
    element.addEventListener('click', handleFieldRemoved);
  });
*/
...
// Save CSV Result
var saveCSVResults = () => {
  var source = document.getElementById(gResultingCSVInput).value;
